...
| `/ops` | 임원 | **지회 운영 허브** — 매뉴얼(4) + 서류함(전자서명·보관 서류) 카드 (탭 "지회 운영") |
| `/ops/docs` `/api/docs/[id]` | 임원 | 보관 서류: 서명 완료본 자동 + 직접 업로드(`chapter_docs`, DB base64 ≤5MB) · 분류 칩 · 열기/다운로드/수정/삭제 |
| `/manual` `/manual/[key]` | 임원 | 운영 매뉴얼 섹션(문서·체크리스트). `/manual` 은 `/ops` 로 리다이렉트 |
| `/sign` `/sign/new` `/sign/[id]` | 임원 | 전자서명(e-Sign): 요청 목록·마법사·현황판. 진입은 지회 운영 허브 (독립 탭 없음) |
| `/s/[token]` `/api/sign/*` | 공개 | 서명자 화면(토큰 게이트 RPC)·서명본 합성 PDF |
