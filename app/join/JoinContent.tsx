// 입회 안내 콘텐츠(공용) — /join 단독 페이지 + 공지(NoticesBoard) '입회안내' 탭에서 함께 사용.
//   훅 없는 순수 컴포넌트 → 서버/클라이언트 양쪽에서 import 가능.
...
// embedded=true → 공지 탭 안에 들어갈 때(상단 큰 히어로·로그인 링크 생략).
export default function JoinContent({ embedded = false }: { embedded?: boolean }) {
  return (
    <div className={`moim-join${embedded ? " jn-embed" : ""}`}>
      ...
      {!embedded ? (
        ...
        <p className="jn-lead jn-lead-embed">
          CBMC(한국기독실업인회)는 일터를 선교지 삼아 비즈니스 현장에서 그리스도를 증거하는
          기독 실업인들의 모임입니다. 새서울지회는 매주 금요일 정기모임을 중심으로
          신앙과 사업을 함께 나누며 동역합니다.
        </p>
      ...
      <footer className="jn-foot">
        ...
        {!embedded && <Link href="/" className="jn-login"><LogIn size={16} /> 이미 회원이신가요? 로그인</Link>}
      </footer>
    </div>
  );
}
...
.moim-join.jn-embed{ padding:0 0 24px; max-width:760px; }
...
.moim-join .jn-lead-embed{ font-size:15px; max-width:none; margin:0 0 4px; }

.moim-join .jn-flow{ display:flex; align-items:stretch; gap:10px; margin:18px 0 30px; }
