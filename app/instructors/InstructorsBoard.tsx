...
// 강사풀 — 외부/내부 강사 풀(회원과 분리). 자기입력 폼(회원형) + 운영진 추가/수정/삭제. 운영진 전용(서버 게이트).
import { useMemo, useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { Plus, Phone, Mail, Trash2, GraduationCap, Download, Upload, Pencil } from "lucide-react";
...
export type Instructor = {
  id: string; name: string; kind: string | null; is_external: boolean;
  org: string | null; phone: string | null; field: string | null; fee_note: string | null; note: string | null;
  email: string | null; company: string | null; position: string | null; intro: string | null;
  photo_url: string | null; business_card_url: string | null;
};
...
export default function InstructorsBoard({ rows, canEdit = true }: { rows: Instructor[]; canEdit?: boolean }) {
  ...
  function startEdit(i: Instructor) {
    setEditId(i.id);
    setForm({ name: i.name, kind: "강사", is_external: i.is_external, org: i.org ?? i.company ?? "", phone: i.phone ?? "", field: i.field ?? "", fee_note: i.fee_note ?? "", note: i.note ?? "" });
    setMsg(""); setShow(true);
    if (typeof window !== "undefined") window.scrollTo({ top: 0, behavior: "smooth" });
  }

  const list = useMemo(() => rows.filter((r) => !fExt || (fExt === "외부" ? r.is_external : !r.is_external)), [rows, fExt]);
  const nExt = rows.filter((r) => r.is_external).length;
  const nInt = rows.length - nExt;
  ...
  function downloadForm() { downloadXlsx(rows.length ? rows.map((r) => tplRow(r)) : [tplRow()], "강사풀_양식"); }
  ...
  return (
    <div className="text-ink">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h1 className="text-[clamp(21px,5vw,26px)] font-extrabold tracking-tight">강사풀</h1>
          <p className="mt-1 text-[14px] font-medium text-ink-soft">강사 {rows.length}명 · 외부 {nExt} · 내부 {nInt} · 회원과 분리된 풀</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {canEdit && <button onClick={() => { try { navigator.clipboard.writeText(`${location.origin}/instructor-form`); setMsg("✅ 정보요청 링크 복사됨 — 강사님께 보내세요"); } catch {} }} className="inline-flex min-h-[42px] items-center gap-1.5 rounded-full border border-primary/40 bg-[rgba(0,102,204,.06)] px-4 text-[14px] font-semibold text-primary hover:bg-[rgba(0,102,204,.12)]"><GraduationCap size={16} /> 정보요청 링크</button>}
      ...
      {canEdit && show && (
        <div className="mb-5 rounded-xl border border-primary/40 bg-[rgba(0,102,204,.04)] p-5">
          <h3 className="mb-3 text-[15px] font-extrabold">{editId ? "강사 수정" : "강사 추가"}</h3>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-1.5">
        <span className="mr-1 text-[12.5px] font-bold text-ink-soft">소속</span>
      ...
      {list.length === 0 ? (
        <p className="rounded-xl border border-line bg-card px-4 py-12 text-center text-[15px] text-ink-soft">등록된 강사가 없어요. ‘정보요청 링크’를 보내거나 ‘추가’로 시작하세요.</p>
      ) : (
              ...
              <div key={i.id} className="relative rounded-xl border border-line bg-card p-4">
                ...
                <div className="mb-1.5 flex items-center gap-2 pr-6">
                  {i.photo_url ? (
                    ...
                    <img src={i.photo_url} alt={i.name} className="h-10 w-10 flex-shrink-0 rounded-lg object-cover" />
                  ) : <span className="grid h-10 w-10 flex-shrink-0 place-items-center rounded-lg bg-[rgba(0,102,204,.1)] text-primary"><GraduationCap size={18} /></span>}
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5"><span className="text-[16px] font-bold">{i.name}</span>
                    ...
                    {(org || i.position) && <div className="truncate text-[13px] text-ink-soft">{[org, i.position].filter(Boolean).join(" · ")}</div>}
                  </div>
                </div>
                {i.field && <div className="mt-1 text-[13.5px] text-ink">📚 {i.field}</div>}
                {i.phone && <a href={`tel:${i.phone}`} className="mt-1 mr-3 inline-flex items-center gap-1 text-[13.5px] font-semibold text-primary"><Phone size={13} /> {i.phone}</a>}
                {i.email && <a href={`mailto:${i.email}`} className="mt-1 inline-flex items-center gap-1 text-[13.5px] font-semibold text-primary"><Mail size={13} /> {i.email}</a>}
                {i.intro && <div className="mt-1 text-[13px] text-ink-soft">{i.intro}</div>}
                {i.fee_note && <div className="mt-1 text-[13px] text-ink-soft">💰 {i.fee_note}</div>}
