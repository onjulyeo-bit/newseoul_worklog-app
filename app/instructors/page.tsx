// 강사·간사 (서버) — instructors 불러오기 + 운영진 확인.
...
export default async function InstructorsPage() {
  ...
  const { data } = await supabase.from("instructors").select("id, name, kind, is_external, org, phone, field, fee_note, note").eq("chapter_id", "새서울").order("created_at", { ascending: false });
  return <InstructorsBoard rows={(data as Instructor[]) ?? []} />;
}
