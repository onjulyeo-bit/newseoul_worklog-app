...
export default async function SignStatusPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: req }, { data: slots }, { data: signers }, { data: sent }, gt] = await Promise.all([
    ...
    supabase.from("sign_events").select("signer_id, event").eq("request_id", id).in("event", ["link_sent", "phone_consent"]),
  ...
  const ev = (sent ?? []) as Array<{ signer_id: string | null; event: string }>;
  const sentIds = new Set(ev.filter((e) => e.event === "link_sent").map((e) => e.signer_id).filter(Boolean) as string[]);
  // 유선 동의로 기록된 서명자 (setPhoneConsent) — 되돌리면 'reverted' 이벤트가 쌓이므로 현재 상태와 함께 판단
  const phoneIds = new Set(ev.filter((e) => e.event === "phone_consent").map((e) => e.signer_id).filter(Boolean) as string[]);
  return (
    <SignStatus
