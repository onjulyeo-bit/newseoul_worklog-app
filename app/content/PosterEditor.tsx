...
const THEMES = [
  { key: "navy", label: "남색", bg: "linear-gradient(135deg,#1e2353,#1e2353,#1e2353)" },
...
const SWATCHES = ["#ffffff", "#1e2353", "#9db8e8", "#2e7d52", "#ffd9a8", "#ecd29a", "#ffe3b3", "#c0392b", "#000000"];
// 단색 배경용 팔레트
...
export default function PosterEditor({ seed, publish }: { seed: Seed; publish?: { title: string; body: string } }) {
  ...
  // 회원 공지에 게시 (포스터 PNG + 안내글)
  const [publishing, setPublishing] = useState(false);
  const [published, setPublished] = useState(false);
  async function publishNotice() {
    if (!publish) return;
    if (!confirm("이 포스터와 안내글을 회원 공지에 게시할까요?")) return;
    setPublishing(true);
    try {
      ...
      if (posterRef.current) {
        ...
        if (!upErr) imageUrl = supabase.storage.from("posters").getPublicUrl(fname).data.publicUrl;
      }
      const { error } = await supabase.from("announcements").insert({ category: "주간모임", title: publish.title, body: publish.body, image_url: imageUrl });
              ...
              <div>
                ...
                <div className="mt-1 flex flex-wrap items-center gap-1.5">
                  <label className="flex cursor-pointer items-center gap-1 rounded-md border border-line bg-card px-1.5 py-1 text-[11px] font-semibold text-ink-soft">
                    🎨 색선택
                    <input type="color" value={bgColor || "#1e2353"} onChange={(e) => { setBgColor(e.target.value); setBgImage(""); }} className="h-6 w-7 cursor-pointer border-0 bg-transparent p-0" />
                  </label>
                  {SOLIDS.map((c) => (
