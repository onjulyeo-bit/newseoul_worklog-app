          ...
          <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(perYear.length, 3)},minmax(0,1fr))`, gap: 12, marginBottom: 20 }}>
            {perYear.map((p) => (
              <div key={p.year} className="card" style={{ padding: 16 }}>
                ...
                <div className="dy-row"><Users size={15} color="#0066cc" /><span className="dy-n">{p.payers}</span><span className="dy-l">명 납부</span></div>
...
.moim-fin .dy-n{ font-size:22px; font-weight:800; letter-spacing:-.03em; color:#0052a8; }
