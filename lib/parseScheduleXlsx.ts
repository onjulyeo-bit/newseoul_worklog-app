...
export function parseScheduleXlsx(buf: ArrayBuffer): Row[] {
  ...
  const iDate = find("날짜"), iSession = find("회차"), iMode = find("모드", "구분"), iTitle = find("주제", "제목"),
    iSpeaker = find("강사", "발제", "진행"), iHost = find("사회자", "사회"), iProgram = find("모임형식", "형식", "프로그램"),
    iVerse = find("성경본문", "성경", "본문"), iPraise = find("찬양"), iNote = find("비고");
  ...
  for (let r = h + 1; r < rows.length; r++) {
    ...
    out.push({ date, nth, mode, session, title: clean(cell(iTitle)) ?? "", speaker: clean(cell(iSpeaker)) ?? "", host: clean(cell(iHost)) ?? "", verse: clean(cell(iVerse)) ?? "", praise: clean(cell(iPraise)) ?? "", note: clean(cell(iNote)) ?? "", program: clean(cell(iProgram)) ?? "" });
  }
